      ...
      <table class="table table-striped b-t b-light">
        ...
        @foreach($all_product as $key =>$pro)
          <tr>
            ...
            <td>{{$pro->product_name}}</td>s
            <td>{{$pro->product_price}}</td>
            <td>{{$pro->product_desc}}</td>
            <td>{{$pro->product_content}}</td>
            <td>{{$pro->category_name}}</td>
            <td>{{$pro->brand_name}}</td>
            <td><img src="public/upload/product/{{$pro->product_image}}" style="height:70px; width:70px;" alt=""></td>
            <td><span class="text-ellipsis">
            <?php
            if($pro->product_status==0)
            {?>
              <a href="{{URL::to('/onactive-product/'.$pro->product_id)}}"><i class="fa fa-eye"></i></a>
            <?php
            }
            else  
            {?>
